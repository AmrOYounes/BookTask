...
export const exportData = (config) => {
  ...
  return axios.get(`/export?search_by_word=${config.params.search_by_word}&type=${config.params.type}&column_filter=${config.params.column_filter}&unit_start=${config.params.unit_start}&unit_end=${config.params.unit_end}&price_start=${config.params.price_start}&price_end=${config.params.price_end}`,{ Authorization: `bearer ${token}`,responseType: 'blob' });
   
}

export const ordersExPORT = config => {
  return axios.get(`/Ordersexport?search_by_word=${config.params.search_by_word}&type=${config.params.type}&column_filter=${config.params.column_filter}`,{ Authorization: `bearer ${token}`,responseType: 'blob' });
